...
public class AudioObjectPool : MonoBehaviour {
    ...
    //can be called anywhere to take a source from the active list 
    public AudioSource GrabSource()
    {
        AudioSource source;

        //there is an inactive source
        if (inactiveAudioSources.Count > 0)
        {
            source = inactiveAudioSources[0].GetComponent<AudioSource>();
            activeAudioSources.Add(source.gameObject);
            inactiveAudioSources.RemoveAt(0);
        }
        //no inactive source, generate a new one
        else
        {
            GameObject newSourceObj = Instantiate(audioObjPrefab, transform.position, Quaternion.identity, transform);
            source = newSourceObj.GetComponent<AudioSource>();
            activeAudioSources.Add(source.gameObject);
        }
    ...
    //can be called from any AudioManager to return an active source to inactive list
    public void ReturnSource(AudioSource source)
    {
        RemoveActiveAudio(source.gameObject);
    }
    ...
    public void RemoveActiveAudio(GameObject obj)
    {
        ...
        activeAudioSources[index].transform.SetParent(transform);
