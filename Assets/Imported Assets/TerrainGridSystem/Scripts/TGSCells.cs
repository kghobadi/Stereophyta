...
namespace TGS {
	...
	public partial class TerrainGridSystem : MonoBehaviour {
        ...
        [SerializeField]
        Color
            _cellHighlightColor2 = new Color(0, 1, 0, 0.9f);
		...
		/// </summary>
		public List<int> CellGetNeighbours (Cell cell, int maxDistance, int cellGroupMask = -1, int maxSearchCost = 0, bool ignoreCanCrossCell = false) {
			int cellIndex = CellGetIndex (cell);
			return CellGetNeighbours (cellIndex, maxDistance, cellGroupMask, maxSearchCost, ignoreCanCrossCell);
		}

		/// <summary>
		/// Get a list of cells which are nearer than a given distance in cell count
		/// </summary>
		public List<int> CellGetNeighbours (int cellIndex, int maxDistance, int cellGroupMask = -1, int maxSearchCost = 0, bool ignoreCanCrossCheck = false) {
			if (cellIndex < 0 || cellIndex >= cells.Count)
				return null;
				...
				if (cx >= 0 && cx < _cellColumnCount && cy >= 0 && cy < _cellRowCount) {
					...
					if (cells [ci].iteration == cellIteration) {
						...
						if (distanceFunction (cellIndex, ci) <= maxDistance) {
							List<int> steps = FindPath (cellIndex, ci, out dummyCost, maxSearchCost, maxDistance, cellGroupMask, ignoreCanCrossCheck, ignoreCanCrossCheck);
							if (steps != null) {
								int stepsCount = steps.Count;
								for (int k = 0; k < stepsCount; k++) {
									cells [steps [k]].iteration = cellIteration;
								}
								cc.Add (ci);
							}
						}
