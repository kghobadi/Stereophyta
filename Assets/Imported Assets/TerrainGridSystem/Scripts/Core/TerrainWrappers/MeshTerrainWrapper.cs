...
namespace TGS {
    public class MeshTerrainWrapper : ITerrainWrapper {
        ...
        public float SampleHeight(Vector3 worldPosition) {
            if (worldPosition.x < boundsMin.x || worldPosition.x >= boundsMax.x || worldPosition.z < boundsMin.z || worldPosition.z >= boundsMax.z)
                return 0;

            // Depth texture is squared so we need to fit within bounds
            float size = boundsSize.z > boundsSize.x ? boundsSize.z : boundsSize.x;
            float yPos = _heightmapHeight * (worldPosition.z - (boundsCenter.z - size * 0.5f)) / size;
            float xPos = _heightmapWidth * (worldPosition.x - (boundsCenter.x - size * 0.5f)) / size;
        ...
        public Vector3 GetInterpolatedNormal(float x, float y) {
            ...
            float size = boundsSize.z > boundsSize.x ? boundsSize.z : boundsSize.x;
            x = (x - 5f) * boundsSize.x / size + 0.5f;
            y = (y - 5f) * boundsSize.z / size + 0.5f;
            if (x < 0 || x >= 1f || y < 0 || y >= 1)
                return Misc.Vector3up;

            float yPos = y * _heightmapHeight;
            float xPos = x * _heightmapWidth;
            int yPos0 = (int)(yPos);
            int xPos0 = (int)(xPos);
            int yPos1 = yPos0 == _heightmapHeight - 1 ? yPos0 : yPos0 + 1;
            int xPos1 = xPos0 == _heightmapWidth - 1 ? xPos0 : xPos0 + 1;
        ...
        public Vector3 localCenter {
            get {
                float y = (boundsMin.y - _gameObject.transform.position.y) / _gameObject.transform.lossyScale.y;
        ...
        void ComputeDepth() {
            ...
            Texture2D tex = new Texture2D(_heightmapWidth, _heightmapHeight, TextureFormat.RGBAFloat, false);
            // Read heightmap
