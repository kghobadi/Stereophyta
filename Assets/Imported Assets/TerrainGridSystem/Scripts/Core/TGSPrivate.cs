...
namespace TGS {
	...
	public partial class TerrainGridSystem : MonoBehaviour {
		...
		const string BACKGROUND_BILLBOARD = "Background Billboard";
		Transform backgroundBillboard;
		...
		Dictionary<int, bool> tempDictCells;
		List<int> tempListCells;
		int cellIteration;
		Dictionary<Point, Segment> dictSegments;

		// Z-Figther & LOD
		Vector3 lastLocalPosition, lastCamPos, lastPos, lastScale;
		...
		void GenerateCellsMesh () {
			...
			int meshGroups = (frontiersPointsCount / 65000) + 1;
			int meshIndex = -1;
		...
		void FitToTerrain () {
			...
			bool gridNeedsUpdate = transform.position != lastPos || transform.lossyScale != lastScale;
		...
		void UpdateMaterialDepthOffset () {
			...
			SetBlend (territoriesMat);
			territoriesDisputedMat.SetFloat ("_Offset", depthOffset);
			SetBlend (territoriesDisputedMat);
		...
		void UpdateMaterialNearClipFade () {
			if (_nearClipFadeEnabled && _terrainWrapper != null) {
				...
				territoriesDisputedMat.DisableKeyword (SKW_NEAR_CLIP_FADE);
				foreach (Material mat in frontierColorCache.Values) {
		...
		void DrawTerritoryBorders () {
			...
			for (int t = 0; t < territoryMeshes.Count; t++) {
				...
				for (int k = 0; k < tm.territoryMeshBorders.Length; k++) {
					...
					flayer.transform.localPosition = Vector3.back * 0.001f * _gridMinElevationMultiplier;
					flayer.transform.localRotation = Quaternion.Euler (Misc.Vector3zero);
		...
		GameObject GenerateRegionSurfaceSimpleQuad (Poly2Tri.Polygon poly, Region region, int cacheIndex, Material material, Vector2 textureScale, Vector2 textureOffset, float textureRotation, bool rotateInLocalSpace) {
			...
			Rect rect = (canvasTexture != null && material != null && material.mainTexture == canvasTexture) ? canvasRect : region.rect2D;
			GameObject surf = Drawing.CreateSurface (cacheIndex.ToString (), meshPoints, quadIndices, material, rect, textureScale, textureOffset, textureRotation, rotateInLocalSpace, disposalManager);
			_lastVertexCount += 4;
			surf.transform.SetParent (surfacesLayer.transform, false);
			surfaces [cacheIndex] = surf;
			surf.transform.localPosition = Misc.Vector3zero;
				...
				if (hitCount > 0) {
					for (int k = 0; k < hitCount; k++) {
						if (hits [k].collider.gameObject == _terrainWrapper.gameObject) {
							Vector3 localPoint = _terrainWrapper.transform.InverseTransformPoint (hits [k].point);
							float w = _terrainWrapper.size.x;
							float d = _terrainWrapper.size.z;
...
//				localCenter.y = _terrainWrapper.SampleHeight (_terrainWrapper.transform.TransformPoint (localCenter));
//				return _terrainWrapper.transform.TransformPoint (localCenter);
