...
namespace TGS_Editor {
	...
	public class TGSInspector : Editor {
		...
		public override void OnInspectorGUI () {
			...
			tgs.gridCenter = EditorGUILayout.Vector2Field ("Center", tgs.gridCenter);
			...
			tgs.gridMeshDepthOffset = EditorGUILayout.IntSlider ("Mesh Depth Offset", tgs.gridMeshDepthOffset, -200, 200);
			tgs.gridSurfaceDepthOffset = EditorGUILayout.IntSlider ("Cells Depth Offset", tgs.gridSurfaceDepthOffset, -200, 200);
			tgs.gridSurfaceDepthOffsetTerritory = EditorGUILayout.IntSlider ("Territories Depth Offset", tgs.gridSurfaceDepthOffsetTerritory, -200, 200);
            ...
            if (tgs.terrain != null) {
				...
				if (tgs.nearClipFadeEnabled) {
					tgs.nearClipFade = EditorGUILayout.Slider ("   Distance", tgs.nearClipFade, 0.0f, 100.0f);
					tgs.nearClipFadeFallOff = EditorGUILayout.Slider ("   FallOff", tgs.nearClipFadeFallOff, 0.001f, 100.0f);
				}
			}

			tgs.transparentBackground = EditorGUILayout.Toggle ("No Background", tgs.transparentBackground);
			if (tgs.transparentBackground) {
				EditorGUILayout.HelpBox ("In transparent mode the Grid will be visible even if there's no geometry in the background.", MessageType.Info);
			}
			...
			if (!Application.isPlaying) {
				...
				if (tgs.enableGridEditor) {
					...
					if (selectedCount == 0) {
						...
						if (GUILayout.Button ("Update Color & Texture")) {
							for (int k = 0; k < selectedCount; k++) {
